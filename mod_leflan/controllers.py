...
def r():
    ...
    return render_template(
        "leflan/post.html", html=html, title="recent updates", t=_t("recent updates")
    )
...
def topuria():
    ...
    html = get_html(page)
    return render_template(
        "leflan/post.html", html=html, title="topuria", t=_t("topuria")
    )
...
def language(language):
    ...
    html = get_html(page)
    return render_template(
        "leflan/post.html", html=html, title=_title(language), t=_t(language)
    )
...
def through_index(language, learning_type):
    ...
    return render_template(
        "leflan/post.html", html=html, title=_title(title), t=_t(title)
    )
...
def dictionary(language):
    ...
    for row in input_file_dict:
        try:
            ...
            entry = {"headword": row, "senses": [], "tags": ["error"], "extra": ""}
            tag_list.add("error")
        ...
        for _sense in _senses:
            ...
            try:
                senses.append(
                    {"meaning": _parts[0], "example": _parts[1], "english": _parts[2]}
                )
            except IndexError:
                senses.append({"meaning": _parts[0], "example": "", "english": ""})

        if "（" in data[0]:
            matches = re.match(r"(.*?)\[(.+?)\]（([\u3040-\u3096]+?)）(.*)", data[0])
            headword = "{0}<ruby><rb>{1}</rb><rt>{2}</rt></ruby>{3}".format(
                matches.group(1), matches.group(2), matches.group(3), matches.group(4)
            )
        else:
            headword = data[0]

        entry = {"headword": headword, "senses": senses, "tags": tags, "extra": extra}
        dictionary.append(entry)

    # remove the beginning html which messes up sorting for Japanese words
    dictionary = sorted(
        dictionary, key=lambda entry: entry["headword"].replace("<ruby><rb>", "")
    )
    tag_list = sorted(tag_list)

    return render_template(
        "leflan/dictionary.html",
...
def book(book):
    ...
    html = get_html(page)
    return render_template(
        "leflan/post.html", html=html, title=_title(book), t=_t(book)
    )
    ...
    for filename in sorted(
        ...
        if len(_parts) == 2 and _parts[0] != ".DS":
            ...
            if not category or tag in verb_to_tag[category]:
                ...
                if show_tags:
                    html += u'<li><a href="%s">%s</a> <tag>%s</tag></li>' % (
                        url,
                        page_name,
                        tag,
                    )
