...
if not testing:
  host   = 'zmnebi.com'
  if not (app.config['ZMNEBI_TEST'] or app.config['LEFLAN_TEST'] or app.config['PARRYC_TEST'] or app.config['CORBIN_TEST'] or app.config['KHACHAPURI_TEST'] or app.config['AVAR_TEST']):
...
def favicon():
  return send_from_directory(os.path.join(app.root_path, 'static'),
                             'favicon_zmna.ico', mimetype='image/vnd.microsoft.icon')
...
def r():
  ...
  html = get_html(page)
  return render_template('zmnebi/post.html',html=html,title='recent updates',t="Georgian Verbs | ქართული ზმნები")
